...
Available data do not measure the object at the center of the model: the share of
AI research performed by AI systems rather than humans. They do, however, separate
some of the model's inputs and outputs. The physical resources devoted to AI are
expanding, and the quality-adjusted services obtained from those resources are
expanding faster. These facts motivate the distinction between raw compute and
the services produced by compute; they do not identify either substitution
elasticity.
...
from 1.09 million to 10.7 million H100-equivalent processors. Over the same two
years, their quality-adjusted inference and training indexes increased by factors
...
humans with eight hours \citep{metrclaude2025}. The model, scaffolding, tasks, and
time budgets differ, so these results are not a comparable time series. They
...
evidence motivates the state variables and the two uses of compute, but it does
not calibrate the feedback strength or the probability of an accelerating path.
...
and labor alone. If production labor remains a positive fraction of the
population, output per capita is constant along the resulting balanced path. At
unit elasticity, the paper derives the growth rates and resource
shares of balanced-growth candidates. Automating research tightens the relevant
feedback condition because capability now raises both inference services and
automated research services. When the elasticity exceeds one and AI services
asymptotically dominate final production, unbounded capability raises output
relative to capital and hence the return to capital without bound. A finite-rate
balanced-growth path is impossible.
...
remaining interior, converging to the postulated ratios, and satisfying the
monopoly second-order condition. Under those conditions,
...
AI-dominated limit, rather than inferring wage behavior from labor's share. These
results connect the literature on
