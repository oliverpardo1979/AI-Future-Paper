...
equal their analytical candidate values. At every collocation evaluation it
solves the monopoly supply condition, labor-market clearing, and the dual research
problem. Residual-controlled collocation then imposes the four equations in
\eqref{eq:axm-canonical-system}; both direct research first-order conditions are
reconstructed separately in the audit.
...
\subsubsection{Unit-elasticity equilibrium-system audit}
...
An independent script reconstructs the static equations from every saved row
without importing the solver and differentiates the four state and jump
variables using a centered seven-point rule. All prespecified acceptance gates
pass; the report contains 130 rows, including diagnostics and the overall result.
Across the two primary paths, the largest independently reconstructed static
error is \(2.92\times10^{-11}\), the largest dynamic error is
\(5.68\times10^{-7}\), and the largest monopoly-FOC log error is
\(2.50\times10^{-11}\). The maximum collocation RMS error is
\(9.43\times10^{-7}\); no accepted observation uses numerical clipping or a
fallback root. Across the three horizons, the range of either initial jump is
less than \(7.7\times10^{-8}\) log points. The finite-date TVC proxies also pass
the stated truncation gates. These checks establish accurate finite-horizon
solutions of the dated candidate system, not existence or transversality of an
exact infinite-horizon equilibrium.
...
Figure \ref{fig:axm-complement-shares} shows why the macroeconomic limit is not
a failure to automate research. In the gross-substitutes research case, the
automated research-expenditure share moves from near zero to near one. At the
same time, the technological AI share inside the labor--AI composite moves
toward an interior value and both labor-income measures move toward the positive
limits in Propositions \ref{prop:axm-complements} and
\ref{prop:axm-complements-research}.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures_axm/axm_complements_factor_shares_automation.png}
\caption{Factor shares and research automation. Here \(s_X\) is AI services'
technological share within the labor--AI composite, not their final-output
income share; \(s_M\) is the automated research-expenditure share. Horizontal
reference lines are conditional analytical limits.}
\label{fig:axm-complement-shares}
\end{figure}

Finally, Figure \ref{fig:axm-complement-research} displays a transition hidden
by the limiting shares. Human research first rises as a fraction of population
and then vanishes; raw inference and research compute also vanish relative to
output. Capability nevertheless keeps rising because each unit of research
compute produces \(A\) units of automated research services.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures_axm/axm_complements_research_resources.png}
\caption{Capability, human research, and raw-compute use under final-production
complementarity. Capability is shown as a natural-log change from date zero;
the remaining panels are percentages. Positive allocations at every saved
finite date preserve interiority even as the displayed ratios approach zero.}
...
Figure \ref{fig:axm-feedback-diagnostic} evaluates the Davidson-style
balanced-growth accounting at the dated value of \(s_M(t)\) along the same
numerical paths. With Cobb--Douglas research, the plug-in gain is constant at
\(G=0.196875\). With substitutable research, \(s_M\) rises and the plug-in measure
\(G(t)=\eta s_M(t)(1+\nu)\) increases from 0.0621 to 0.5625. It remains below one.
This is not the dated feedback Jacobian: away from balanced growth, \(\nu\) is not
the contemporaneous transmission coefficient from capability to output. The
calculation is therefore a diagnostic, not a stand-alone explosion test or
...
accounting in \eqref{eq:axm-feedback-matrix}, not a sufficient condition for
explosive equilibrium dynamics.}
