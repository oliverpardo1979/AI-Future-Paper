...
\(\sigma_{HM}>1\) &
Same production bottleneck; \(s_M\to1\) if \(Aw/\xi\to\infty\) &
...
\begin{equation}
 ...
 =\left(\frac{\omega_M}{\omega_H}\right)^{\sigma_{HM}}
  \left(\frac{Aw}{\xi}\right)^{\sigma_{HM}},
...
\begin{equation}
 \frac{s_M}{1-s_M}
 =\left(\frac{\omega_M}{\omega_H}\right)^{\sigma_{HM}}
  \left(\frac{Aw}{\xi}\right)^{\sigma_{HM}-1}.
 \label{eq:axm-automation-odds}
\end{equation}
The relevant relative price is \(Aw/\xi\): the wage relative to the cost of one
unit of automated research services.

\begin{proposition}[Research automation]
\label{prop:axm-research-automation}
Along any interior path on which \(Aw/\xi\to\infty\), the automated contribution
...
\begin{equation}
 Y^{1-\beta}
 =K^\alpha L^\lambda\left(\frac{\beta^2A}{\xi}\right)^\beta.
...
\begin{equation}
 ...
 \begin{cases}
  \omega_M,&\sigma_{HM}=1,\\
  1,&\sigma_{HM}>1\text{ and }Aw/\xi\to\infty.
 \end{cases}
 \label{eq:axm-sbar}
\end{equation}
...
\item If \(\sigma_{HM}=1\), every balanced-growth equilibrium must have
 \(D(\omega_M)>0\).
\item If \(\sigma_{HM}>1\), \(Aw/\xi\to\infty\), and the path converges to an
...
\begin{align}
 \frac{g_A}{z}&\to h,&
 \frac{\xi U}{Y}&\to(1-\alpha)^2,\\
 \frac{\dot K+\delta K}{Y}&\to\alpha-\theta h,&
 \frac{\xi M}{Y}&\to\frac{\eta(1-\alpha)^2}{\Delta},\\
