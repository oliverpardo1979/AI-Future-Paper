...
consumption FOC imply, for any admissible alternative \(\widetilde K^{s}\) with the
...
\[
 ...
 \geq \Lambda_T(\widetilde K_T^{s}-(K_T^{s})^*)
 \geq-\Lambda_T(K_T^{s})^*\to0,
\]
where the last limit is the household transversality condition. In equilibrium,
capital-market clearing identifies \(K^s=K\). Convergence of
...
\begin{equation}
 ...
 \qquad
 \Gamma_t=m\left(\frac{\omega_H}{\omega_M}\right)^{\sigma_{HM}}
...
\eqref{eq:axm-app-sectoral-labor} that \(\Gamma_t\to\Gamma>0\) and
\(g_{\Gamma_t}=o(z)\). Moreover,
\[
...
\[
 g_y=g_{\Gamma_t}+\kappa(g_{\mathcal Q}-g_L),\qquad
 n=g_L+a_Hg_y.
\]
Consequently,
\[
 \frac{g_L}{z}
 =
 \frac{n/z-a_H\{g_{\Gamma_t}/z+\kappa g_{\mathcal Q}/z\}}
 {1-\kappa a_H}.
\]
