...
\]
Because \(s_M/(1-s_M)=(\xi M)/(wH)=(\xi/Aw)(AM/H)\), substitution yields
\[
 \frac{s_M}{1-s_M}
 =\left(\frac{\omega_M}{\omega_H}\right)^{\sigma_{HM}}
...
monopoly allocation,
\[
 p_X\left[1-\frac{1-s_X}{\sigma_{XL}}-\alpha s_X\right]=\frac{\xi}{A}.
\]
...
condition becomes \(\beta p_X=\xi/A\). Since \(p_XX=\beta Y\),
\(\xi X/A=\beta^2Y\), proving \eqref{eq:axm-inference-share-cd}. Replacing
\(X\) by \(\beta^2AY/\xi\) gives \eqref{eq:axm-reduced-cd}. Taking growth rates
...
investment share in \eqref{eq:axm-cd-shares}. Let \(m=\xi M/Y\) and
\(u=\beta^2\). The research-compute first-order condition can be written
\[
 q\,\eta\bar s\,\frac{\dot A}{M}=\xi,
 \qquad\text{so}\qquad
...
Let \(z=Y/K\), \(u=\xi U/Y\), \(m=\xi M/Y\), \(i=(\dot K+\delta K)/Y\), and
