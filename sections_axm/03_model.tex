...
The paid inputs are \(U\) and \(M\). Because both uses have the same constant unit cost, we choose
...
\end{equation}
At \(\sigma_{HM}=1\), its continuous limit is
\begin{equation}
...
\(H\), uses \(M\), and owns \(A\). At each date it faces the inverse demand
schedule implied by final-good firms and takes the wage and aggregate state as
given. Its intertemporal problem is
\begin{equation}
 \max_{\{X,H,M\}}
 \int_0^\infty
...
developer. Along the household Euler equation, this discounting is equivalent to
using the household's stochastic discount factor. The inverse elasticity of
demand for \(X\) is
\begin{equation}
...
conditions, the research first-order
...
by the terminal conditions. The block is closed by initial \(K,A,N\), the labor and goods market
...
\begin{equation}
 ...
 \label{eq:axm-tvc}
\end{equation}
The numerical boundary conditions select the admissible transition approaching the
...
constants. An accelerating path has an output growth rate that does not converge
to a finite constant. Following \citet{aghionjonesjones2019}, a
