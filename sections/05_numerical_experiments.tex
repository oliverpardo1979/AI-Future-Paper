...
The experiments follow the analytical ordering in
Section~\ref{sec:analytical-results}: the cases $\sigma_{XL}<1$ and
$\sigma_{XL}=1$ are presented first, followed by $\sigma_{XL}>1$. The figures
illustrate equilibrium transitions and quantify outcomes that the propositions do
not sign. They do not convert necessary conditions or conditional asymptotic
characterizations into existence theorems.
...
values. Table~\ref{tab:targeted-configurations} selects five configurations that
separate the economically distinct mechanisms. The first, second, third, and fifth
configurations have canonical equilibrium paths. The fourth is an analytical stress
test: without an economically justified terminal condition, a reduced singular path
...
\begin{table}[H]
    ...
    \begin{tabular}{clcccc}
        \toprule
        & Configuration & $\sigma_{XL}$ & $\sigma_{HM}$ & Feedback region & Numerical status \\
        \midrule
        1 & Labor bottleneck & 0.75 & 2.00 & $D_{\mathrm{AI}}>0$ & canonical equilibrium \\
        2 & Human-essential research & 1.00 & 1.00 & $D_{\mathrm{CD}}>0$ & canonical equilibrium \\
        3 & Gradual research automation & 1.00 & 2.00 & $D_{\mathrm{AI}}>0$ & canonical equilibrium \\
        4 & Strong research feedback & 1.00 & 2.00 & $D_{\mathrm{CD}}>0>D_{\mathrm{AI}}$ & analytical stress test \\
        5 & Production-side acceleration & 1.35 & 2.00 & $D_{\mathrm{AI}}>0$ & free-boundary equilibrium \\
        \bottomrule
    \end{tabular}}
    \begin{minipage}{0.96\textwidth}
    \footnotesize\emph{Notes:} Configurations 1--3 and 5 hold all parameters and
    initial stocks in Table~\ref{tab:equilibrium-numerical-parameters} fixed. In
    configuration 4, setting $\phi=0.90$ while retaining $\eta=0.45$ gives
    ...
    configuration 5, not separate economic regimes.
    \end{minipage}
\end{table}
...
$\sigma_{HM}=2$. The human-essential path is shown separately below so that the
research-technology comparison does not obscure the production-regime comparison.
...
\subsection{Human-essential and substitutable research at $\sigma_{XL}=1$}
...
When $\sigma_{HM}=2$, the automated contribution rises toward one and human
research employment converges to zero. The feedback denominator falls from its
human-dominated value toward $D_{\mathrm{AI}}$. Capability growth consequently
converges to 2.274 percent and per-capita output growth to 0.568 percent. The
comparison therefore isolates two effects of gross substitution in research: it
changes who performs research and strengthens the feedback from output to capability.
...
\subsection{Production gross substitution and equilibrium acceleration}
...
\subsection{What the simulations establish---and what they do not}
...
\begin{enumerate}[label=(\roman*)]
    ...
    condition.
    \item The $\sigma_{XL}=1$, $\sigma_{HM}=2$ path approaches the rates in conditional
    result~\ref{cond:ai-dominated-cd-limit} and is likewise stable when the
    ...
    existence theorem.
    \item The $\sigma_{XL}=\sigma_{HM}=1$ path approaches the fully Cobb--Douglas
    ...
    \item For $\sigma_{XL}>1$, the paths remain interior over their computed domains
    ...
    negative-denominator branches. Configuration 4 identifies a parameter region
    to be solved in future work; it is not plotted as an equilibrium path here.
\end{enumerate}
