...
The recovery is not an analytical theorem. In the most elastic paths, the human
research share of population approaches one over the computed transition, so
production labor approaches an allocation boundary. This near-corner reallocation
is the immediate source of labor scarcity and is also a reason not to extrapolate
the recovery to every equilibrium. Moving the terminal value of $Y/K$ from 3 to 10
...
These checks make the temporary decline robust within the selected conditional
branch. They do not establish that every admissible branch eventually recovers or
rule out another equilibrium with a persistent wage decline.
