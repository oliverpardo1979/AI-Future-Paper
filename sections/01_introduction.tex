...
There is a sharper result under Cobb--Douglas research and the fixed-spending
closure used in the numerical experiments. The human-research wage bill is then a
constant share of output, so the production and aggregate labor-income shares rise
together when $\sigma_{XL}<1$ and remain constant when $\sigma_{XL}=1$.
...
The toy model delivers eight results. First, the automated share of research follows
...
between output and research. Sixth, human essentiality in research does not restore
balanced growth when gross substitution in production makes the output--capital
ratio rise with capability. Seventh, the production CES separates a
labor-bottlenecked constant-growth limit with zero per-capita growth, a
constant-share Cobb--Douglas benchmark, and a regime in which the return to capital
diverges with unbounded capability and no interior steady state exists. The break at
unit elasticity is a singular perturbation, so the gross-substitution regime may
remain quantitatively remote when $\sigma_{XL}$ is close to one. Eighth, the production structure
...
worker reallocation between production and research. Under Cobb--Douglas research,
the fixed-spending closure removes this additional margin by holding the
human-research wage bill constant as a share of output.
...
illustrative parameters. The central gross-substitution experiment sets
$\sigma_{XL}=2$, without relying on the earlier $\sigma_{XL}=4$ extreme. Output- and
consumption-per-capita growth initially fall, reach minima after approximately five
decades, and then reaccelerate until the path reaches the numerical capital-growth
cutoff in year 196. A sufficiently strong feedback from output to automated research
generates a separate acceleration mechanism even under Cobb--Douglas production.
Near-unit experiments sharpen the asymptotic qualification. With
$\sigma_{XL}=1.01$, the economy remains close to the Cobb--Douglas path after one
thousand simulated years; with $\sigma_{XL}=1.05$, reacceleration is visible but the
path remains below the cutoff; and with $\sigma_{XL}=1.10$, the path reaches the
cutoff in year 744.
The wage experiments reject an unconditional wage-growth result. With high
population growth and a very high production elasticity $\sigma_{XL}$, the displacement
...
rise. Remark~\ref{rem:numerical-wage-counterexample} reports an explicit
parametrization and decomposes the negative wage-growth rate into these two terms.
The labor-share experiments show that Cobb--Douglas production can keep the
