...
\]
For finite $w_t/p^X_t>0$, the right-hand side converges to
$\omega_X/\omega_L$ as $\sigma_{XL}\to1$, which implies $s^X_t\to\omega_X$.
At $\sigma_{XL}=1$, the exponent on the relative price is zero and the share is
identically $\omega_X$. For every fixed $\sigma_{XL}>1$, the odds diverge when
$w_t/p^X_t\to\infty$, so $s^X_t\to1$. These observations establish the two
iterated limits in \eqref{eq:noncommuting-production-limits}.

Finally, set $s^X_t=\bar s$ in \eqref{eq:production-share-odds} and solve for
$w_t/p^X_t$ to obtain \eqref{eq:production-share-threshold}. Because
$\bar s>\omega_X$,
\[
...
$1/(\sigma_{XL}-1)$ as $\sigma_{XL}\downarrow1$.
\end{proof}
...
\end{equation}
Proposition~\ref{prop:production-ces-regimes} establishes $s^X_t\to1$ under
...
is a positive constant, then $A_t\to\infty$, so the final claim follows.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:high-sigma-singular-scaling}]
...
Euler equation gives $g_C/z\to\alpha$. A constant consumption share therefore
requires $g_Y/z\to\alpha$. Because
...
\end{align}
Because $u_t$ converges to a positive constant,
\begin{equation}
    \bar u
    =
...
Finally, automated research dominance and a constant research-resource share imply
...
\]
But $g_A=\bar a z$ asymptotically and $g_z=\vartheta g_A$, so the left-hand side
also converges to $\vartheta\bar a$. Equating the two expressions yields
\[
    \bar a
    =
...
\begin{proof}[Proof of Proposition~\ref{prop:nonunit-ces-steady-states}]
Consider first $\sigma_{XL}<1$. Proposition~\ref{prop:production-ces-regimes}
