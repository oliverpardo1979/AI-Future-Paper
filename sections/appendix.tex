...
\]
If $\theta>1$ and $w_t/\xi\to\infty$, the right-hand side diverges and
...
\eqref{eq:automation-odds} remain bounded, so $\theta>1$ does not by itself imply
$s^M_t\to1$.
\end{proof}
...
\[
    ...
    \varepsilon(s)
    =
    \frac{1-s}{\sigma}+\alpha s.
\]

For $\sigma<1$, revenue approaches zero both as $x_t\to0$ and as
...
\[
    \bar s^X=\frac{1-\sigma}{1-\alpha\sigma}.
\]
Because the CES mapping between $s^X$ and $X/L_Y$ is one-to-one, the corresponding
service ratio is finite.

For $\sigma=1$, the CES contribution $s^X_t$ equals the constant weight $\omega$.

For $\sigma>1$, revenue is unbounded as $x_t\to\infty$ when marginal cost is zero,
...
\[
    ...
    \sim
    \left[
        \frac{\omega^{\sigma/(\sigma-1)}(1-\alpha)^2}{\xi}
    \right]^{(1-\alpha)/\alpha}
    A_t^{(1-\alpha)/\alpha}.
\]
...
$r-\delta_K=\rho+\gamma^\infty$ and $g_Y=n+\gamma^\infty$, and substituting the
