...
\]
Their difference identifies a parameter region in which balanced growth is possible
while humans remain essential but disappears when automated research removes that
bottleneck. Because the CES research share changes endogenously, the model also
derives an exact path condition for finite-time singularity rather than classifying
the economy only by its human- and AI-dominated endpoints.

The third departure concerns the interaction between recursive innovation and the
production CES. The paper proves that, for every fixed $\sigma_{XL}>1$, unbounded
capability makes the return to capital diverge and rules out an interior steady state
with finite constant growth rates. It also shows that the unit-elasticity boundary is
