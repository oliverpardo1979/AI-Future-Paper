...
Let $q_t\equiv w_t/p^X_t$ and suppose $\omega_L,\omega_X>0$ with
...
Consequently:
\begin{enumerate}[label=(\roman*)]
    \item for every finite $q_t>0$, $s^X_t$ is continuous in $\sigma_{XL}$ at one
    and converges to $\omega_X$ as $\sigma_{XL}\to1$;
    \item if $q_t\to\infty$, then $s^X_t=\omega_X$ when $\sigma_{XL}=1$, whereas
    $s^X_t\to1$ for every fixed $\sigma_{XL}>1$; hence
    \begin{equation}
        \lim_{\sigma_{XL}\downarrow1}\lim_{q\to\infty}s^X(q,\sigma_{XL})=1,
        \qquad
        \lim_{q\to\infty}\lim_{\sigma_{XL}\downarrow1}s^X(q,\sigma_{XL})
        =\omega_X;
    ...
    \end{equation}
    and $q_{\bar s}(\sigma_{XL})\to\infty$ as
...
elasticity is close to one. Section~\ref{sec:numerical} quantifies this transition.
...
remains interior.
\begin{enumerate}[label=(\roman*)]
    \item If $\sigma_{XL}<1$ and \eqref{eq:vanishing-normalized-ai-cost} holds, any
    constant-growth path with constant $K/Y$, $H/N$, and $\xi M/Y$, and with
    positive human and automated research inputs, satisfies
    \begin{equation}
