...
\begin{align}
    ...
    &=
    \left[
        (1-\omega)L_{Y,t}^{(\sigma_{XL}-1)/\sigma_{XL}}
        +\omega X_t^{(\sigma_{XL}-1)/\sigma_{XL}}
    \right]^{\sigma_{XL}/(\sigma_{XL}-1)},
    \qquad
    \omega\in(0,1),\quad \sigma_{XL}>0.
...
Cobb--Douglas composite is obtained by continuity when $\sigma_{XL}=1$.

Define the contribution of AI services to the CES composite as
\begin{equation}
    s^X_t
    \equiv
    \frac{\omega X_t^{(\sigma_{XL}-1)/\sigma_{XL}}}
    {(1-\omega)L_{Y,t}^{(\sigma_{XL}-1)/\sigma_{XL}}
        +\omega X_t^{(\sigma_{XL}-1)/\sigma_{XL}}}.
...
\begin{align}
    ...
    &=
    \left[
        (1-\nu)H_t^{(\sigma_{HM}-1)/\sigma_{HM}}
        +\nu M_t^{(\sigma_{HM}-1)/\sigma_{HM}}
    \right]^{\sigma_{HM}/(\sigma_{HM}-1)},
    \qquad
    \nu\in(0,1),\quad \sigma_{HM}>1.
...
The elasticity $\sigma_{HM}$ governs substitution between human and automated research.
...
$\sigma_{XL}$ governs substitution between $L_{Y,t}$ and $X_t$. The research elasticity
...
$L_{Y,t}=(1-h_t)N_t$, $g_{L_Y,t}=n-\dot h_t/(1-h_t)$.
...
\begin{equation}
    E_{H,t}
    =(1-\nu)\left(\frac{E_t}{H_t}\right)^{1/\sigma_{HM}},
    \qquad
    E_{M,t}
    =\nu\left(\frac{E_t}{M_t}\right)^{1/\sigma_{HM}}.
...
\begin{equation}
    ...
    =
    \left[
        (1-\nu)^{\sigma_{HM}} w_t^{1-\sigma_{HM}}
        +\nu^{\sigma_{HM}}\xi^{1-\sigma_{HM}}
    \right]^{1/(1-\sigma_{HM})}.
...
automated research. A smaller CES weight $\nu$ reinforces this initial human
...
To isolate the role of human essentiality, consider instead the counterfactual
\begin{equation}
    E_t^{\mathrm{CD}}
    =
    H_t^{1-\nu}M_t^\nu.
...
\end{equation}
Both inputs are essential in \eqref{eq:cd-research-counterfactual}. Along an interior
balanced-growth path, a constant allocation of population implies $g_H=n$, while a
constant automated-research resource share implies $g_M=n+\gamma$. Effective
research therefore grows at
\begin{equation}
    g_E^{\mathrm{CD}}
    =
    n+\nu\gamma.
...
\end{equation}
Combining this expression with $\gamma=\upsilon g_A$ gives
\begin{align}
    g_A^{\mathrm{CD}}
    &=
    ...
    \gamma^{\mathrm{CD}}
    &=
    \frac{\upsilon\eta n}{1-\phi-\eta\nu\upsilon}.
    \label{eq:full-cd-gamma}
\end{align}
Define the two feedback denominators
\begin{equation}
    D_{\mathrm{CD}}
    \equiv
    1-\phi-\eta\nu\upsilon,
...
interior. The Cobb--Douglas research counterfactual admits a positive
constant-growth path only if $D_{\mathrm{CD}}>0$. The AI-dominated research limit
admits a positive constant-growth path only if $D_{\mathrm{AI}}>0$. Moreover,
\begin{equation}
...
$D_{\mathrm{CD}}>0\geq D_{\mathrm{AI}}$: the economy has balanced growth if human
research remains essential but no positive constant-growth path after automated
research becomes asymptotically dominant.
\end{proposition}

\begin{proof}
Equations~\eqref{eq:cd-research-growth} and
...
two denominators gives \eqref{eq:feedback-denominator-gap}.
\end{proof}

The denominator condition is necessary but not sufficient for an interior
equilibrium. The automated-research resource share in the Cobb--Douglas
counterfactual is
\begin{equation}
    m_M^{\mathrm{CD}}
    \equiv
...
\end{equation}
An interior path also requires a positive denominator in
\eqref{eq:full-cd-research-resource-share} and a positive consumption share after
replacing $m_R^\infty$ with $m_M^{\mathrm{CD}}$ in
\eqref{eq:bg-consumption-share}.

For Cobb--Douglas research, the automated-research contribution is fixed at $\nu$,
...
For $\sigma_{XL}<1$, a rising wage relative to $\xi$ is not guaranteed: labor remains
...
$s^M=1$ even though $\sigma_{HM}>1$. For $\sigma_{XL}=1$, the positive wage growth in
\eqref{eq:other-bg-rates} makes automated research asymptotically dominant. For
$\sigma_{XL}>1$, production itself becomes an acceleration mechanism. Research
...
\end{equation}
If research is Cobb--Douglas, $E_t=H_t^{1-\nu}M_t^\nu$, the automated contribution
share remains equal to $\nu$ and the human input remains essential. Nevertheless,
...
\end{equation}
and constant $\xi$, any $\nu>0$ preserves a feedback from the scale of output to
...
positive automated-input elasticity $\nu$ still lets the expanding scale of output
raise $M_t$ and therefore research productivity. If $\nu=0$, production growth still
