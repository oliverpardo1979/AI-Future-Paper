...
The proposition can be sharpened into a conditional characterization of constant
growth away from Cobb--Douglas production.
...
remains interior. Consider a constant-growth path with constant $K/Y$, $H/N$, and
$\xi M/Y$, and with positive human and automated research inputs.
\begin{enumerate}[label=(\roman*)]
    \item If $\sigma_{XL}<1$, any such path satisfies
    \begin{equation}
    ...
    \item If $\sigma_{XL}>1$, there is no such path with $g_A>0$ on which all
    aggregate variables grow at finite constant rates.
\end{enumerate}
\end{proposition}
...
The ideas equation then gives $g_A=\eta n/(1-\phi)$. The second claim follows from
\eqref{eq:high-sigma-ak-limit}: if $g_A>0$, $Y/K$ and the return to capital cannot
remain constant.
\end{proof}
