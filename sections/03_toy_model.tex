...
Figure~\ref{fig:model-flow} summarizes the model's technologies, resource allocation,
and state dynamics. The diagram is not a one-way production tree. Final output is
allocated to consumption, investment, inference compute, and automated research;
investment accumulates physical capital, while research improves capability and
therefore feeds back into both AI services and subsequent research. The diagram also
makes explicit the model's second within-period allocation margin: population is
divided between labor in final production and human AI research. Hence, assigning an
additional worker to research reduces the labor available for current production.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        >=Latex,
        font=\small,
        input/.style={draw, rounded corners=2pt, minimum width=2.7cm,
            minimum height=1.0cm, align=center, fill=gray!8},
        state/.style={draw, rounded corners=2pt, minimum width=2.7cm,
            minimum height=1.0cm, align=center, fill=blue!8},
        technology/.style={draw, rounded corners=2pt, minimum width=3.5cm,
            minimum height=1.0cm, align=center, fill=orange!9},
        allocation/.style={draw, rounded corners=2pt, minimum width=3.0cm,
            minimum height=1.0cm, align=center, fill=gray!8},
        flow/.style={->, semithick},
        dynamics/.style={->, semithick, dashed}
    ]
        \node[state]      (N)     at (-10.9, 5.0) {$N_t$\\population};
        \node[allocation, minimum width=4.0cm] (labor) at (-7.0, 5.0)
            {$N_t=L_t+H_t$\\labor allocation};
        \node[input]      (L)     at (-3.0, 6.3) {$L_t$\\production labor};
        \node[input]      (H)     at (-3.0, 2.0) {$H_t$\\human research};

        \node[state]      (A)     at (-7.2,-0.6) {$A_t$\\capability};
        \node[technology] (X)     at (-3.0, 4.0) {$X_t=A_tU_t$\\AI services};
        \node[technology] (Z)     at ( 0.0, 5.2) {$Z_t=\operatorname{CES}(L_t,X_t)$};
        \node[state]      (K)     at ( 0.0, 7.2) {$K_t$\\physical capital};
        \node[technology] (Y)     at ( 2.4, 6.2) {$Y_t=K_t^\alpha Z_t^{1-\alpha}$};
        \node[allocation, minimum width=3.8cm] (goods) at ( 6.2, 6.2)
            {$Y_t=C_t+I_t$\\$+\xi(U_t+M_t)$\\goods allocation};

        \node[allocation] (C)     at ( 9.9, 7.7) {$C_t$\\consumption};
        \node[allocation] (I)     at ( 9.9, 6.2) {$I_t$\\investment};
        \node[input]      (U)     at ( 5.0, 3.8) {$U_t$\\inference compute};
        \node[allocation] (M)     at ( 9.9, 3.0) {$M_t$\\automated research};
        \node[technology] (E)     at ( 1.0, 1.2) {$E_t=\operatorname{CES}(H_t,M_t)$};
        \node[technology] (Adot)  at (-2.0,-0.6)
            {$\dot A_t=\chi A_t^\phi E_t^\eta$};

        \draw[flow] (N) -- (labor);
        \draw[flow] (labor) -- (L);
        \draw[flow] (labor.east) -- (-5.0,2.0) -- (H.west);

        \draw[flow] (A.north) -- (-7.2,4.0) -- (X.west);
        \draw[flow] (U) -- node[below] {inference input} (X);
        \draw[flow] (X) -- (Z);
        \draw[flow] (L) -- (Z);
        \draw[flow] (Z) -- (Y);
        \draw[flow] (K) -- (Y);
        ...
        \draw[flow] (goods) -- (C);
        \draw[flow] (goods) -- (I);
        \draw[flow] (goods) -- node[right] {$\xi U_t$} (U);
        \draw[flow] (goods) -- node[above right] {$\xi M_t$} (M);
        ...
        \draw[dynamics] (I.east) -- (11.8,6.2) -- (11.8,9.1) --
            node[above] {$\dot K_t=I_t-\delta K_t$} (0.0,9.1) -- (K.north);
        \draw[dynamics] (Adot.south) -- ++(0,-1.15) -|
            node[pos=0.42,below] {capability accumulation} (A.south);
    \end{tikzpicture}
    }
    \caption{Technology, resource allocation, and state feedback in the toy model.
    Solid arrows denote within-period production or resource use; dashed arrows
    denote laws of motion. The two allocation nodes distinguish the division of
    population between production and research from the division of output across
    its four uses.}
...
\begin{assumption}[Stable human-essential research benchmark]
\label{ass:stable-human-essential-benchmark}
Throughout the paper, the parameters satisfy
\begin{equation}
    1-\phi
...
asymptotic restrictions in Proposition~\ref{prop:high-sigma-singular-scaling}.
...
\begin{proposition}[AI-dominated limit under Cobb--Douglas production]
\label{prop:ai-dominated-cd-limit}
...
derivation of \eqref{eq:bg-research-resource-share} is reported in
Appendix~\ref{sec:appendix-proofs}. Positive consumption is necessary for an interior
household allocation.
\end{proof}

The proposition is a necessary characterization of the AI-dominated limit. It does
...
loop. A zero or negative fully Cobb--Douglas denominator is excluded throughout.
...
interior. Under Assumption~\ref{ass:stable-human-essential-benchmark}, the fully
Cobb--Douglas model admits a positive constant-growth path only if
\eqref{eq:fully-cd-consumption-feasibility} holds. The
...
$\eta n/D_{\mathrm{CD}}$. A zero or negative $D_{\mathrm{CD}}$ is therefore excluded.
The acceleration and singularity cases below concern the endogenous denominator
$D(s^M_t)$ as research becomes automated; that denominator can become nonpositive
even though $D_{\mathrm{CD}}>0$.
...
technologies admit balanced growth. Under a high-feedback mechanism calibration,
$D_{\mathrm{CD}}=0.08575$ but
...
Consequently:
\begin{enumerate}[label=(\roman*)]
    \item for every finite $\mathcal R_t>0$, $s^X_t$ is continuous in $\sigma_{XL}$ at one
    and converges to $\omega_X$ as $\sigma_{XL}\to1$;
    \item if $\mathcal R_t\to\infty$, then $s^X_t=\omega_X$ when $\sigma_{XL}=1$, whereas
    $s^X_t\to1$ for every fixed $\sigma_{XL}>1$; hence
    \begin{equation}
    ...
    $\sigma_{XL}>1$, the relative price required to attain $s^X=\bar s$ is
    \begin{equation}
        \mathcal R_{\bar s}(\sigma_{XL})
        =
        \left[
            \frac{\bar s}{1-\bar s}
            \left(\frac{\omega_L}{\omega_X}\right)^{\sigma_{XL}}
        \right]^{1/(\sigma_{XL}-1)},
        \label{eq:production-share-threshold}
    \end{equation}
    and $\mathcal R_{\bar s}(\sigma_{XL})\to\infty$ as
    $\sigma_{XL}\downarrow1$.
\end{enumerate}
\end{proposition}

Proposition~\ref{prop:production-unit-elasticity-knife-edge} explains why the
Cobb--Douglas result and the gross-substitution result are not contradictory. At
...
an unbounded increase in $w/p^X$ eventually makes AI services dominant. The limits
...
The absence of a finite-growth steady state leaves open the form of an accelerating
equilibrium. The following proposition characterizes its limiting ratios. It is a
conditional result: it describes an interior path that reaches an AI-dominated
finite-time singularity, but does not by itself prove that the path exists from
...
\begin{proposition}[Conditional singular-equilibrium scaling]
\label{prop:high-sigma-singular-scaling}
...
terminal value relative to consumption at the singular boundary.
\end{proposition}
