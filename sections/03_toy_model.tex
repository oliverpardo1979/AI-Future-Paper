...
therefore feeds back into both AI services and subsequent research.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[state]      (A)    at (-6.5, 0.0) {$A_t$\\capability};
        \node[technology] (X)    at (-4.0, 2.4) {$X_t=A_tU_t$\\AI services};
        \node[input]      (L)    at (-4.0, 4.7) {$L_t$\\production labor};
        \node[technology] (Z)    at (-1.0, 3.5) {$Z_t=\operatorname{CES}(L_t,X_t)$};
        \node[state]      (K)    at (-1.0, 5.8) {$K_t$\\physical capital};
        \node[technology] (Y)    at ( 2.0, 4.7) {$Y_t=K_t^\alpha Z_t^{1-\alpha}$};

        \node[allocation] (C)    at ( 6.5, 6.5) {$C_t$\\consumption};
        \node[allocation] (I)    at ( 6.5, 4.9) {$I_t$\\investment};
        \node[input]      (U)    at ( 6.5, 3.3) {$U_t$\\inference compute};
        \node[allocation] (M)    at ( 6.5, 1.7) {$M_t$\\automated research};
        \node[input]      (H)    at ( 6.5,-0.3) {$H_t$\\human research};
        \node[technology] (E)    at ( 1.5,-0.3) {$E_t=\operatorname{CES}(H_t,M_t)$};
        \node[technology] (Adot) at (-2.5,-0.3)
            {$\dot A_t=\chi A_t^\phi E_t^\eta$};

        \draw[flow] (A) -- (X);
        \draw[flow] (U.east) -- (8.4,3.3) -- (8.4,-2.5) --
            node[below] {inference input} (-8.4,-2.5) -| (X.west);
        \draw[flow] (X) -- (Z);
        \draw[flow] (L) -- (Z);
        \draw[flow] (Z) -- (Y);
        \draw[flow] (K) -- (Y);
        ...
        \draw[flow] (Y) -- (C);
        \draw[flow] (Y) -- (I);
        \draw[flow] (Y) -- node[above] {$\xi U_t$} (U);
        \draw[flow] (Y) -- node[above right] {$\xi M_t$} (M);
        ...
        \draw[dynamics] (I.east) -- (8.4,4.9) -- (8.4,7.7) --
            node[above] {$\dot K_t=I_t-\delta K_t$} (-1.0,7.7) -- (K.north);
    ...
    denote laws of motion. Final output satisfies
    $Y_t=C_t+I_t+\xi(U_t+M_t)$, and labor satisfies $L_t+H_t=N_t$.}
