...
\midrule
$\sigma_{HM}=1$
& Labor bottleneck with human-essential research.
& Fully Cobb--Douglas benchmark; $D_{\mathrm{CD}}$ governs reduced recursive feedback.
& Unbounded-capability production mechanism with human-essential research. \\
\addlinespace
$\sigma_{HM}>1$
& Labor bottleneck with an endogenous transition toward automated research.
& Constant production shares with a changing research-feedback denominator.
& Unbounded-capability production mechanism combined with potentially AI-dominated research. \\
\bottomrule
\end{tabular}
...
\footnotesize
\textit{Notes:} The entries identify the mechanism to be studied in each technology
class; they do not assume that an endogenous input ratio reaches its asymptotic
limit. When both nests are Cobb--Douglas, $D_{\mathrm{CD}}$ is constant. With CES
research, the feedback denominator changes with the automated-research contribution.
When $\sigma_{XL}\neq1$, the production share also changes, so $D_{\mathrm{CD}}$ is
a benchmark rather than a global classifier.
\end{minipage}
\end{table}
