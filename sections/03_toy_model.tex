...
$\sigma$ governs substitution between $L_{Y,t}$ and $X_t$. The research elasticity
$\theta$ governs substitution between $H_t$ and $M_t$. The variable $E_t$ is the
...
when $\sigma<1$, is constant when $\sigma=1$, and falls when $\sigma>1$. The
...
the same growth rate and the familiar threshold $\sigma=1$ applies to both.
...
\begin{equation}
    ...
    \equiv
    \left(\frac{\nu}{1-\nu}\right)^\theta
    \left(\frac{w_0}{\xi_0}\right)^{\theta-1}.
...
$s^M_t\to1$. A larger $\theta$ or a faster increase in the research wage relative
...
\begin{equation}
    g_{h,t}
    =
    \theta\left(g_{\xi,t}-g_{w,t}\right)
    +g_{M,t}-n,
    \qquad
...
\begin{equation}
    g_{M,t}-n
    >
    \theta\left(g_{w,t}-g_{\xi,t}\right).
...
\begin{equation}
    g_{h,t}
    =
    g_{Y,t}-n-\theta g_{w,t}+(\theta-1)g_{\xi,t}.
...
\begin{equation}
    g_{w,t}-g_{\xi,t}
    =
    \frac{g_{M,t}-g_{H,t}}{\theta}
    =
    \frac{\upsilon}{\theta}g_{A,t}.
...
\begin{equation}
    ...
    =
    s^M_t(1-s^M_t)
    \frac{\theta-1}{\theta}\upsilon g_{A,t}.
...
A finite-time singularity occurs if there is a finite date $T^*$ such that
$g_{A,t}$ and $A_t$ diverge as $t\uparrow T^*$. Accelerating growth that remains
finite at every finite date is not a finite-time singularity.
\end{definition}
...
the fixed elasticity $\theta>1$ permits a transition from the human-dominated growth
